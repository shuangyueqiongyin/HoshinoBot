...
@on_command(
    'zai?', aliases=('在?', '在？', '在吗', '在么？', '在嘛', '在嘛？'), only_to_me=True)
...
@sv.on_prefix(('沙雕机器人', '沙雕機器人'), only_to_me=True)
...
@sv.on_prefix('mua', only_to_me=True)
...
@sv.on_prefix(('我有个朋友说他好了', '我朋友说他好了', ))
...
async def chat_clanba(bot, ctx):
    if random.random() < 0.33:
        await bot.send(ctx, R.img('快出刀啊.jpg').cqcode)
